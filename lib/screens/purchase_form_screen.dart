...
class _DraftItem {
  ...
  double get lineTotal =>
      PurchaseMath.lineTotal(quantity: quantity, unitCost: unitCost);
}
...
class _PurchaseFormScreenState extends State<PurchaseFormScreen> {
  ...
  double get _subtotal =>
      PurchaseMath.subtotal(_items.map((item) => PurchaseItem(
  ...
  double get _grandTotal => PurchaseMath.grandTotal(
        subtotal: _subtotal,
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      if (_isEditing) {
        final purchase = await _purchaseService.getById(widget.purchaseId!);
        if (!purchase.canEdit) {
          throw const AppException('Only draft purchases can be edited.');
        }
        _companyId = purchase.companyId;
        _purchaseDate = purchase.purchaseDate;
        _invoice.text = purchase.invoiceNumber ?? '';
        _reference.text = purchase.referenceNumber ?? '';
        _notes.text = purchase.notes ?? '';
        _discount.text = purchase.discount.toStringAsFixed(2);
        _otherCharges.text = purchase.otherCharges.toStringAsFixed(2);

        if (!_companies.any((c) => c.id == purchase.companyId)) {
          try {
            _companies = [
              ..._companies,
              await _companyService.getById(purchase.companyId),
            ];
          } catch (_) {}
        }

        await _loadProducts(purchase.companyId);
  ...
  Future<void> _addItem() async {
    ...
    final costController = TextEditingController(
      text: selected.purchasePrice.toStringAsFixed(2),
    );
                  ...
                  child: Column(
                    ...
                    children: [
                      AppDropdown<Product>(
                        ...
                        onChanged: (value) {
                          ...
                          setLocal(() {
                            selected = value;
                            costController.text =
                                value.purchasePrice.toStringAsFixed(2);
                          });
                        },
                      ),
                      ...
                      AppTextField(
                        ...
                        validator: (value) =>
                            Validators.nonNegativeNumber(value, 'Unit cost'),
                      ),
                    ],
                  ),
                ),
    ...
    final unitCost = double.parse(costController.text.trim());
    final existingIndex =
    ...
    setState(() {
      if (existingIndex >= 0) {
        ...
        existing.quantity = PurchaseMath.roundMoney(
    ...
    if (merged && mounted) {
      AppSnackbar.show(
        context,
        'Product already on this purchase. Quantity updated.',
      );
    }
  }
  ...
  Future<void> _editItem(_DraftItem item) async {
    ...
    final costController =
        TextEditingController(text: item.unitCost.toStringAsFixed(2));
    final formKey = GlobalKey<FormState>();
    ...
    setState(() {
      ...
      item.unitCost = double.parse(costController.text.trim());
    });
  }
  ...
  Future<void> _save({required String status}) async {
    ...
    try {
      if (_isEditing) {
        await _purchaseService.updateDraft(
      ...
      body: _loading
          ...
          : Form(
              ...
              child: ListView(
                children: [
                  AppSectionHeader(
                    title: _isEditing ? 'Edit draft purchase' : 'New purchase',
                    subtitle:
                      ...
                      children: [
                        ...
                        Align(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            label: Text(Formatters.date(_purchaseDate)),
                          ),
                        ),
                        const SizedBox(height: AppSizes.md),
                        _twoCol(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  Text('Cost ${Formatters.money(item.unitCost)}'),
